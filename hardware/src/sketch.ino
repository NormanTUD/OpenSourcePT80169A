...
void insert_book () {
	...
	toparm_links();
	linken_aussenarm_runter();
	sprintf(printableText, "done insert"); myPrint();
}
...
void roller_normalstellung() {
	...
	if(roller_is_normalstellung != true || roller_is_normalstellung == NULL) {
		...
		roller_links_mit_zeit(100);
		roller_is_normalstellung = true;
	}
	intendation_level--;
}

void aussenarm_normalstellung() {
	rechten_aussenarm_runter();
	linken_aussenarm_runter();
}
...
void toparm_links () {
	...
	toparm_is_right = false;
	toparm_is_normalstellung = false;
	sprintf(printableText, "done toparm left"); myPrint();
	intendation_level--;
}
...
void toparm_links_rest () {
	intendation_level++;
#if enabledebug
	sprintf(printableText, "toparm_links_rest"); debug();
#endif
